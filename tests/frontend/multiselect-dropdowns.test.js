...
describe('Multiselect Dropdowns', () => {
  ...
  describe('Filtering Behavior', () => {
    test('filters repositories by selected trending timeframes', () => {
      // Mock repository data
      const mockRepos = [
        { id: 1, source: 'trending-daily', stars: 500 },
        { id: 2, source: 'trending-weekly', stars: 1500 },
        { id: 3, source: 'trending-monthly', stars: 5000 },
      ];

      // This will be implemented in the filtering logic
      const dropdownButton = screen.getByTestId('trending-dropdown-button');
      fireEvent.click(dropdownButton);
      ...
      // Should only show daily trending repos
      const reposList = screen.getByTestId('repositories-list');
      expect(reposList.children).toHaveLength(1);
    });

    test('filters repositories by selected star ranges', () => {
      // Mock repository data
      const mockRepos = [
        { id: 1, source: 'trending-daily', stars: 50 },
        { id: 2, source: 'trending-daily', stars: 500 },
        { id: 3, source: 'trending-daily', stars: 5000 },
        { id: 4, source: 'trending-daily', stars: 15000 },
      ];
      ...
      // Should only show repos with 1K-10K stars
      const reposList = screen.getByTestId('repositories-list');
      expect(reposList.children).toHaveLength(1);
    });

    test('combines trending and stars filters with AND logic', () => {
      // Mock repository data
      const mockRepos = [
        { id: 1, source: 'trending-daily', stars: 500 },
        { id: 2, source: 'trending-daily', stars: 5000 },
        { id: 3, source: 'trending-weekly', stars: 5000 },
      ];

      // Select daily trending
      const trendingDropdown = screen.getByTestId('trending-dropdown');
      fireEvent.click(trendingDropdown);
      const dailyCheckbox = screen.getByLabelText('Daily');
      fireEvent.click(dailyCheckbox);

      // Select 1K-10K stars
      const starsDropdown = screen.getByTestId('stars-dropdown');
      fireEvent.click(starsDropdown);
      const range = screen.getByLabelText('1K-10K stars');
      fireEvent.click(range);

      // Should only show repos that are both daily AND in the 1K-10K range
      const reposList = screen.getByTestId('repositories-list');
      expect(reposList.children).toHaveLength(1);
    });

    test('shows all repositories when no filters are selected', () => {
      // Mock repository data
      const mockRepos = [
        { id: 1, source: 'trending-daily', stars: 500 },
        { id: 2, source: 'trending-weekly', stars: 1500 },
      ...
      // No filters selected - should show all repos
      const reposList = screen.getByTestId('repositories-list');
      expect(reposList.children).toHaveLength(3);
    });
  });
});
